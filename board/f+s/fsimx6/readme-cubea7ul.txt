How to build and install rootfs for CUBEA7UL, AGATEWAY, HGATEWAY
---------------------------------------------------------------

Unpack linux sources to a directory:

  tar xvf linux-3.14.52-fsimx6ul-Vx.y.tar.bz2 or
  tar xvf linux-4.1.15-fsimx6ul-Vx.y.tar.bz2

This creates directory linux-3.14.52-fsimx6ul-Vx.y or
directory linux-4.1.15-fsimx6ul-Vx.y, where x.y is the
...
to the buildroot directory. So create the following symbolic link:
  ...
  ln -s linux-3.14.52-fsvimx6ul-Vx.y linux-fsimx6ul or
...
time (or after a "make clean"), this will really use cubea7ul_defconfig,
...
After all is done, you have to call
  ...
  board/f+s/fsvybrid/mkinstall-cubea7ul.bash
...
arch/arm/configs/cubea7ul_defconfig in Linux, call
