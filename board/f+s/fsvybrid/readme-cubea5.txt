...
However the kernel will also automatically be stored in directory
/boot in the root filesystem. It will also automatically create the
script image install-cubea5.scr in the top directory of buildroot.
This script matches the current root filesystem size.
...
MFG-Tool should execute the following steps:
 ...
 1. Erase the flash ('E')
 2. Load install-cubea5.scr to RAM @ 80300000 ('*')
 3. Detect and verify install-cubea5.scr ('%')
...
U-Boot will take over, executes install-cubea5.scr in RAM @ 80300000
