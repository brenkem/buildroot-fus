...
elif "RAUC_MEM_TYPE" not in os.environ.keys():
    print("Variable RAUC_MEM_TYPE is not defined")
    exit(1)

elif "RAUC_TEMPLATE" not in os.environ.keys():
    print("Variable RAUC_TEMPLATE not defined")
    exit(1)

elif "RAUC_PATH_TO_OUTPUT" not in os.environ.keys():
    print("Variable RAUC_PATH_TO_OUTPUT is noz defined")
    exit(1)

path_to_rauc = os.environ["RAUC_BINARY"]
path_to_cert = os.environ["RAUC_CERT"]
path_to_key = os.environ["RAUC_KEY"]
input_dir   = os.path.join(os.environ["RAUC_TEMPLATE"], "*")
...
if os.environ["RAUC_MEM_TYPE"] == "emmc":
...
elif os.environ["RAUC_MEM_TYPE"] == "nand":
    shutil.copyfile(os.path.join(os.environ["RAUC_PATH_TO_OUTPUT"], "Image"), os.path.join(dest_dir, "Image.img"))
...
input_dir = dest_dir
output_file = os.path.join(os.environ["DEPLOY_DIR_IMAGE"], "rauc_update.artifact")
